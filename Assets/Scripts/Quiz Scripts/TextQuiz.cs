...
public class TextQuiz : MonoBehaviour
{
    [Header("Questions")]
    [SerializeField] TextMeshProUGUI questionText;
    [SerializeField] List<TextQuestionSO> questionList;
    TextQuestionSO textQuestionSO;
    ...
    private void GetRandomQuestion()
    {
        ...
        textQuestionSO = questionList[index];

        if (questionList.Contains(textQuestionSO))
        {
            questionList.Remove(textQuestionSO);
        }
    }

    private void DisplayQuestion()
    {
        questionText.text = textQuestionSO.GetQuestion();

        for (int i = 0; i < answerButtons.Length; i++)
        {
            TextMeshProUGUI buttonText = answerButtons[i].GetComponentInChildren<TextMeshProUGUI>();
            buttonText.text = textQuestionSO.GetAnswer(i);
        }
    }
    ...
    private void DisplayAnswer(int index)
    {
        ...
        correctAnswerIndex = textQuestionSO.GetCorrectAnswerIndex();
        string correctAnswerString = textQuestionSO.GetAnswer(correctAnswerIndex);
